    ...
    try {
      ...
      User.updateOne({ _id: id }, { confirmed: true }, function(err, docs) {
        if (err) {
          ...
          return res
            .status(200)
            .send({ message: "Your email adress is confirmed" });
        }
      });
    } catch (err) {
...
exports.sendConfirmationEmail = function(req, res) {
  ...
  if (!email) return res.status(400).send({ message: "Missing email in body" });
  else {
    try {
      User.findOne({ email: email }, function(err, savedUser) {
        if (err || !savedUser) {
          return res
            .status(400)
            .send({ message: "Confirmation email has not been sent" });
        }
        // Create token
        const token = jwt.sign(
          {
